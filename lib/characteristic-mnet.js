...
/**
 * CharacteristicMNET is the wrapper for HomeKit charcteristics with everything needed for MNET use
 */
class CharacteristicMNET {
	/**CharacteristicMNET is the wrapper for HomeKit charcteristics with everything needed for MNET use
	 * @constructor
	...
	constructor (service, config, globs) {
		...
		this.globs = globs;
		this.log = globs.log;
		globs.info("CharacteristicMNET.Constructor");
		...
		if (this.chr.props.format===this.globs.Characteristic.Formats.UINT8 || this.chr.props.format===this.globs.Characteristic.Formats.INT ) {
			if (this.config.ValidValues!=undefined) // evil twin required for comparing with undefined
			{
				...
				this.globs.info("Setting validValues to "+ this.config.ValidValues);
			}
		...
		// if the characteristic is writable (that is, from homekit to the MNET bus) AND we have a Set section in the config...
		if (this.chr.props.perms.indexOf(this.globs.Characteristic.Perms.WRITE) > -1) {
			if (this.config.Set) {
				...
				// for each address, make an object {address: string, reversed: boolean}
				//iterate(this.config.Set); //DEBUG
				
				for (var isetga = 0; isetga < this.config.Set.length; isetga++) {
					var cGA = this.config.Set[isetga];
					// if (mnetAccess.validateAddressText(cGA)==='OK') {
						this.setGroupAddressList.push({
							/** @type {string} */
							address: cGA, 
							reversed: (this.config.Reverse===true),
							dptype: this.getDPT()
					 	});
					// } else {
					// 	// scream!
					// 	this.globs.log(mnetAccess.validateAddressText(cGA));
					// 	throw (new Error("CONFIG ERROR").message='Invald group Address: '+ cGA);
					// }
				} // for

				//Default handler, writes MNET values to "Set" section only
				if (this.service.handler === "Default") {
					// bind the mnet write handler to the set event
					this.getHomekitCharacteristic().on("set", this.defaultUpdateMNETValue.bind(this));
				} 
			}
			if (this.service.handler !== "Default") {
		...
		if (this.chr.props.perms.indexOf(this.globs.Characteristic.Perms.READ) > -1) {
			if (this.config.Listen) {
				...
				for (var isetga = 0; isetga < this.config.Listen.length; isetga++) {
					...
					// 	// scream!
					// 	this.globs.log(mnetAccess.validateAddressText(cGA));
					// 	throw (new Error("CONFIG ERROR").message('Invalid group Address: '+ cGA));
					// }
				} // for
				//Default handler
				if (this.service.handler === "Default") {
					...
					for (var ibGA = 0; ibGA < this.listenGroupAddressList.length; ibGA++) {
						...
						this.globs.mnetmonitor.registerGA(cBGA.address, function(val, src, dest, type) {
							mnetAccess.writeValueHK(val, this, this.getDPT(), cBGA.reversed);
						}.bind(this));
					}
				} else {
					// bind to custom API
					for (var isga = 0; isga < this.listenGroupAddressList.length; isga++) {
						...
						this.globs.mnetmonitor.registerGA(thisGA.address, function(val, src, dest, type) {
							this.service.customServiceAPI.mnetbusEventCatcher (this.name, val, src, dest, type);
						}.bind(this));	
					}
	...
	/**
	 * getHomekitCharacteristic extracts the Homekit-Characteristic object from the MNET wrapper object.
	 * 
	...
	/**
	 * get the MNET dpt of the characteristic. Assume based on homekit characteristic type or take from config (has
	 * precedence)
	 * 
	 * @returns {string}
	 */
...
//		this.log("getDPT() for " + this.chr.displayName + ", config says " + this.config.DPT + ", format says " + this.chr.props.format);
	...
	/**
	 * The default handler for characteristic updates to the MNET bus if no special service handler is used. Writes to
	 ...
	 */
	defaultUpdateMNETValue(value, callback, context) {
		// is the value to be sent to the bus, or originating there?
		if (context === 'fromMNETBus') { // from the bus, ignore!
			if (callback) {
				try {
					callback();
				} catch (e) {
					this.globs.log('Caught error '+ e + ' when calling homebridge callback.');
				}
			}
			return;
		}
		...
		for (var int2 = 0; int2 < this.setGroupAddressList.length; int2++) {
			...
			// console.log(value);
			mnetAccess.writeValueMNET(value, this.setGroupAddressList[int2], callback);
		}
...
module.exports = CharacteristicMNET;